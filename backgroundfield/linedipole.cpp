...
double LineDipole::call( double x, double y, double z) const
{
   ...
   double r2 = r[0]*r[0]+r[1]*r[1]; // r[1] not necessary in this case, removed to enable proper cylindrical ionosphere (ionosphere.geometry = 3)
...
//    const double rdotq=q[0]*r[0] + q[1]*r[1] +q[2]*r[2];
   ...
   const double DerivativeSameComponent=D*( 2*r[1]*(r[1]*r[1]-3*r[0]*r[0]))/r6;
   const double DerivativeDiffComponent=D*( 2*r[0]*(r[0]*r[0]-3*r[1]*r[1]))/r6;
   //const double B;
   //const double der;
