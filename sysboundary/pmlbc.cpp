...
/*!\file PmlBC.cpp
 * \brief Implementation of the class SysBoundaryCondition::PmlBC. 
...
namespace SBC
{
    PmlBC::PmlBC() : SysBoundaryCondition() {}
    PmlBC::~PmlBC() {}

	 void PmlBC::addParameters(){
		
		Readparameters::add("PML.precedence", "Number of Gridpoints for X+ PML Layer", 3);
		Readparameters::add("PML.WidthXp", "Number of Gridpoints for X+ PML Layer", 0.0);
		Readparameters::add("PML.WidthYp", "Number of Gridpoints for Y+ PML Layer", 0.0);
		Readparameters::add("PML.WidthZp", "Number of Gridpoints for Z+ PML Layer", 0.0);
		Readparameters::add("PML.WidthXm", "Number of Gridpoints for X- PML Layer", 0.0);
		Readparameters::add("PML.WidthYm", "Number of Gridpoints for Y- PML Layer", 0.0);
		Readparameters::add("PML.WidthZm", "Number of Gridpoints for Z- PML Layer", 0.0);
		// std::cout << "Add Params" << std::endl;
	 }

	void PmlBC::getParameters(){
		// std::cout << "GETParams" << std::endl;

		int myRank;
      MPI_Comm_rank(MPI_COMM_WORLD,&myRank);
      
		if(!Readparameters::get("PML.precedence",  this->precedence )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthXp",  this->PMLSize.find("X+")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthYp",  this->PMLSize.find("Y+")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthZp",  this->PMLSize.find("Z+")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthXm",  this->PMLSize.find("X-")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthYm",  this->PMLSize.find("Y-")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
		if(!Readparameters::get("PML.WidthZm",  this->PMLSize.find("Z-")->second )) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
	
		// std::cout << this->PMLSize.find("X+")->second << std::endl;
		// std::cout << this->PMLSize.find("Y-")->second << std::endl;
	 }


    bool PmlBC::initSysBoundary(
            creal &t,
            Project &project)
    {
   ...
   bool PmlBC::assignSysBoundary(dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
      ...
      for(const auto& dccrgId : cells) {
         ...
         creal z = cellParams[CellParams::ZCRD] + 0.5*dz;
			...
			// Build Bool do Assign
			int firstPmlCell = 2;
			int lastPmlCellXp = this->PMLSize.find("X+")->second; 
			int lastPmlCellYp = this->PMLSize.find("Y+")->second; 
			int lastPmlCellZp = this->PMLSize.find("Z+")->second; 
			int lastPmlCellXm = this->PMLSize.find("X-")->second; 
    ...
    //   Assign boundary flags to local fsgrid cells
      ...
      for (int k=0; k<gridDims[2]; k++) {
         for (int j=0; j<gridDims[1]; j++) {
            for (int i=0; i<gridDims[0]; i++) {
               ...
               std::array<int32_t, 3> pos = technicalGrid.getGlobalIndices(i, j, k);
      ...
      return true;
            ...
            Project &project)
        {
            ...
            {
                if (!setCellsFromTemplate(mpiGrid, popID))
                    success = false;
            }
            if (!setBFromTemplate(mpiGrid, perBGrid))
                success = false;

            return success;
        }

        Real PmlBC::fieldSolverBoundaryCondMagneticField(
            FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, 2> & perBGrid,
        ...
        void PmlBC::fieldSolverBoundaryCondElectricField(
            FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, 2> & EGrid,
            cint i,
            cint j,
            cint k,
        ...
        void PmlBC::fieldSolverBoundaryCondHallElectricField(
        ...
        void PmlBC::fieldSolverBoundaryCondGradPeElectricField(
        ...
        void PmlBC::fieldSolverBoundaryCondDerivatives(
        ...
        void PmlBC::fieldSolverBoundaryCondBVOLDerivatives(
        ...
        void PmlBC::vlasovBoundaryCondition(
            const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid,
            const CellID &cellID,
            const uint popID)
        {
        ...
        bool PmlBC::setBFromTemplate(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid,
                                     FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, 2> &perBGrid)
        {
        ...
        bool PmlBC::setCellsFromTemplate(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid, const uint popID)
        {
            vector<CellID> cells = mpiGrid.get_cells();
#pragma omp parallel for
    ...
    void PmlBC::getFaces(bool *faces)
    {
        for (uint i = 0; i < 6; i++)
            faces[i] = facesToProcess[i];
    }

    bool PmlBC::loadInputData(const uint popID)
    {
        PmlBCSpeciesParameters &sP = speciesParams[popID];
    ...
    */
    vector<vector<Real>> PmlBC::loadFile(const char *fn, unsigned int nParams)
    {
        ...
        if (myRank == 0)
            cout << "Parameter data file (" << fn << ") has " << nlines << " values" << endl;
    ...
    */
//     bool PmlBC::generateTemplateCells(creal &t)
//     {
// #pragma omp parallel for
//         for (uint i = 0; i < 6; i++)
//         {
...
//     }
    ...
    */
    ...
    std::string PmlBC::getName() const { return "PmlBC"; }

    uint PmlBC::getIndex() const { return sysboundarytype::PML; }
