...
namespace SBC {
   OutflowPML::OutflowPML(): SysBoundaryCondition() { }
   OutflowPML::~OutflowPML() { }
   
   void OutflowPML::addParameters() {
      const std::string defStr = "Copy";
      Readparameters::addComposing("outflowPML.faceNoFields", "List of faces on which no field outflow boundary conditions are to be applied ([xyz][+-]).");
      Readparameters::add("outflowPML.precedence", "Precedence value of the outflow system boundary condition (integer), the higher the stronger.", 4);
      Readparameters::add("outflowPML.reapplyUponRestart", "If 0 (default), keep going with the state existing in the restart file. If 1, calls again applyInitialState. Can be used to change boundary condition behaviour during a run.", 0);
      ...
      for(uint i=0; i< getObjectWrapper().particleSpecies.size(); i++) {
        ...
        Readparameters::add(pop + "_outflowPML.vlasovScheme_face_x-", "Scheme to use on the face x- (Copy, Limit, None)", defStr);
        Readparameters::add(pop + "_outflowPML.vlasovScheme_face_y+", "Scheme to use on the face y+ (Copy, Limit, None)", defStr);
        Readparameters::add(pop + "_outflowPML.vlasovScheme_face_y-", "Scheme to use on the face y- (Copy, Limit, None)", defStr);
        Readparameters::add(pop + "_outflowPML.vlasovScheme_face_z+", "Scheme to use on the face z+ (Copy, Limit, None)", defStr);
        Readparameters::add(pop + "_outflowPML.vlasovScheme_face_z-", "Scheme to use on the face z- (Copy, Limit, None)", defStr);

        Readparameters::add(pop + "_outflowPML.quench", "Factor by which to quench the inflowing parts of the velocity distribution function.", 1.0);
      }
   }
   
   void OutflowPML::getParameters() {
      int myRank;
      MPI_Comm_rank(MPI_COMM_WORLD,&myRank);
      if(!Readparameters::get("outflowPML.faceNoFields", this->faceNoFieldsList)) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
      if(!Readparameters::get("outflowPML.precedence", precedence)) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      }
      uint reapply;
      if(!Readparameters::get("outflowPML.reapplyUponRestart",reapply)) {
         if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
         exit(1);
      };
      ...
      for(uint i=0; i< getObjectWrapper().particleSpecies.size(); i++) {
        const std::string& pop = getObjectWrapper().particleSpecies[i].name;
        OutflowPMLSpeciesParameters sP;
        ...
        if(!Readparameters::get(pop + "_outflowPML.face", thisSpeciesFaceList)) {
        ...
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_x+", vlasovSysBoundarySchemeName[0])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_x-", vlasovSysBoundarySchemeName[1])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_y+", vlasovSysBoundarySchemeName[2])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_y-", vlasovSysBoundarySchemeName[3])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_z+", vlasovSysBoundarySchemeName[4])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        if(!Readparameters::get(pop + "_outflowPML.vlasovScheme_face_z-", vlasovSysBoundarySchemeName[5])) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added for population " << pop << "!" << endl;
           exit(1);
        }
        for(uint j=0; j<6 ; j++) {
        ...
        if(!Readparameters::get(pop + "_outflowPML.quench", sP.quenchFactor)) {
           if(myRank == MASTER_RANK) cerr << __FILE__ << ":" << __LINE__ << " ERROR: This option has not been added!" << endl;
           exit(1);
        }
   ...
   bool OutflowPML::initSysBoundary(
      creal& t,
      Project &project
   ) {
      ...
      for(uint i=0; i< getObjectWrapper().particleSpecies.size(); i++) {
         OutflowPMLSpeciesParameters& sP = this->speciesParams[i];
   ...
   bool OutflowPML::assignSysBoundary(dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   bool OutflowPML::applyInitialState(
   ...
   Real OutflowPML::fieldSolverBoundaryCondMagneticField(
       FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, 2> &perBGrid,
   ...
   void OutflowPML::fieldSolverBoundaryCondElectricField(
      FsGrid< std::array<Real, fsgrids::efield::N_EFIELD>, 2> & EGrid,
      cint i,
      cint j,
      cint k,
    ...
    * @param cellID
    */
   void OutflowPML::vlasovBoundaryCondition(
      const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
      const CellID& cellID,
      const uint popID
   ) {
//      phiprof::start("vlasovBoundaryCondition (Outflow)");
      ...
      const OutflowPMLSpeciesParameters& sP = this->speciesParams[popID];
...
//      phiprof::stop("vlasovBoundaryCondition (Outflow)");
   ...
   void OutflowPML::getFaces(bool* faces) {
      for(uint i=0; i<6; i++) faces[i] = facesToProcess[i];
   }
   
   std::string OutflowPML::getName() const {return "OutflowPML";}
   uint OutflowPML::getIndex() const {return sysboundarytype::OUTFLOWPML;}
